...
- Data block rendering (extraction tables, RoB traffic lights, GRADE badges, PRISMA flow, manuscript sections)
...
│   ├── requirements.txt
│   ├── Dockerfile                  # Includes sr-skills install step
│   ├── app/
│   │   ├── api/
...
│   │   │       └── fulltext.py     # Full-text OA checker (via sr-skills)
│   │   ├── core/
│   │   │   ├── config.py           # Settings from .env
│   │   │   ├── auth.py             # Supabase JWT validation
│   │   │   ├── logging_config.py   # Structured JSON logging
│   │   │   └── skills/             # 12 SKILL.md prompt dirs (chat route)
│   │   ├── services/
...
│   │       └── nodes/              # 8 stage node implementations (import from sr_skills)
│
├── frontend/
│   ├── package.json
│   ├── src/
...
│       ├── 001_initial_schema.sql  # 8 core tables + RLS
│       └── 002_search_and_articles.sql  # 5 pipeline tables + triggers + RLS
│
└── docs/
...
```bash
# 1. supabase/migrations/000_cleanup_old_schema.sql
# 2. supabase/migrations/001_initial_schema.sql
# 3. supabase/migrations/002_search_and_articles.sql
```
...
# Install sr-skills (private methodology package - editable mode)
...
- **Backend**: Railway (via Dockerfile, sr-skills bundled in image)
- **Frontend**: Vercel
